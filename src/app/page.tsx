...
function Dashboard({ user }: { user: User }) {
  ...
  return <><Nav /><main className="dashboard">
    ...
    <section className="library-section"><div className="section-head"><div><span className="eyebrow">Seu acervo</span><h2>Obras <span className="count">{series.length}</span></h2></div><div className="library-buttons"><button className="secondary-button" onClick={() => setShowCreate(true)}><Plus size={17}/> Nova obra</button><button className="secondary-button" onClick={() => setShowBatchUpload(true)}><UploadCloud size={17}/> Adicionar vários PDFs</button><button className="primary-button add-button" onClick={() => setShowUpload(true)} disabled={uploading}><UploadCloud size={17}/> Adicionar capítulo</button></div></div>
