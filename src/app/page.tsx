...
import { BookOpen, ChevronRight, FilePlus2, FileText, ListChecks, Pencil, Plus, Search, Star, Trash2, UploadCloud, X } from "lucide-react";
...
function Dashboard({ user }: { user: User }) {
  ...
  const recentSeries = recent ? series.find(item => item.id === recent.series_id) : null;
  const recentPercent = recent && recent.total_pages ? Math.min(100, Math.round(progress[recent.id].page_number / recent.total_pages * 100)) : 0;
  const relevantVolumes = volumes.filter(v => v.series_id === selectedSeries);

  return <><Nav /><main className="dashboard">
    {recent && <section className="continue-card"><div className="continue-icon" style={recentSeries && coverUrls[recentSeries.id] ? { backgroundImage: `url("${coverUrls[recentSeries.id]}")` } : undefined}>{!(recentSeries && coverUrls[recentSeries.id]) && <BookOpen size={24}/>}</div><div className="continue-copy"><span className="eyebrow">Continuar lendo</span><strong>{recentSeries?.title || recent.title}</strong><small>{recent.chapter_number !== null ? `Capítulo ${recent.chapter_number}` : `Página ${progress[recent.id].page_number}`}{recent.chapter_title ? ` · ${recent.chapter_title}` : ""}</small><div className="book-progress" aria-label={`${recentPercent}% lido`}><div style={{ width: `${recentPercent}%` }}/></div></div><Link className="continue-link" href={`/read/${recent.id}`}>Continuar <ChevronRight size={18}/></Link></section>}
    <section className="library-section"><div className="section-head"><div><span className="eyebrow">Acervo privado</span><h1>Minha biblioteca <span className="count">{series.length}</span></h1></div><div className="library-buttons"><Link className="manage-link" href="/manage"><ListChecks size={16}/> Gerenciar</Link><details className="add-menu" ref={addMenuRef}><summary><Plus size={17}/> Adicionar</summary><div className="add-menu-panel"><button onClick={() => { addMenuRef.current?.removeAttribute("open"); setShowUpload(true); }} disabled={uploading}>Adicionar capítulo</button><button onClick={() => { addMenuRef.current?.removeAttribute("open"); setShowBatchUpload(true); }}>Adicionar vários PDFs</button><button onClick={() => { addMenuRef.current?.removeAttribute("open"); setShowCreate(true); }}>Nova obra</button></div></details></div></div>
    ...
    {showBatchUpload && <BatchUploadModal ownerId={user.id} series={series} volumes={volumes} onClose={() => setShowBatchUpload(false)} onComplete={load}/>}
