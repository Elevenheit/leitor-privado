...
export function BatchUploadModal({ ownerId, series, volumes, onClose, onComplete }: {
  ...
  async function process(items: BatchDraft[]) {
    ...
    for (const draft of items) {
      ...
      if (draft.seriesId && !seriesItem || draft.volumeId && draft.volumeId !== "new" && !existingVolume || draft.volumeId === "new" && (!draft.seriesId || !draft.volumeNumber || !Number.isFinite(Number(draft.volumeNumber)))) {
        change(draft.id, { status: "error", error: "Revise a obra e o volume selecionados." });
        continue;
      }
      if (draft.chapterNumber && !Number.isFinite(Number(draft.chapterNumber))) {
      ...
      try {
        ...
        if (draft.volumeId === "new") {
          ...
          volumeId = createdVolumes.get(key) || volumes.find(item => item.series_id === draft.seriesId && item.volume_number === Number(draft.volumeNumber))?.id || null;
          if (!volumeId) {
            ...
            volumeId = data.id as string; createdVolumes.set(key, data.id as string);
          }
        }
