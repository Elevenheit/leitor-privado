...
type Operators = { save: number; restore: number; transform: number; paintImageXObject: number; paintInlineImageXObject: number };
...
function isIllustrationPlacement(bounds: Bounds, view: number[]) {
  ...
  return Number.isFinite(coverage) && pageWidth > 0 && pageHeight > 0
    && visibleWidth / pageWidth >= 0.38 && visibleHeight / pageHeight >= 0.19
    && coverage >= 0.10 && coverage <= 0.70
    && visibleWidth / width >= 0.85 && visibleHeight / height >= 0.85
    && aspect >= 0.42 && aspect <= 2.8;
}
...
export function findIllustrationCandidates(page: PDFPageProxy, operatorList: Awaited<ReturnType<PDFPageProxy["getOperatorList"]>>, ops: Operators): Candidate[] {
  ...
  for (let index = 0; index < operatorList.fnArray.length; index++) {
    ...
    if (operator === ops.save) stack.push([...matrix]);
      ...
      if ((typeof source === "string" || (source && typeof source === "object")) && isIllustrationPlacement(bounds, page.view)) {
...
function isImageSource(value: unknown): value is ImageSource {
  ...
  return Number.isInteger(image.width) && Number.isInteger(image.height)
    ...
    && image.width / image.height >= 0.42 && image.width / image.height <= 2.8;
}
...
export function createReaderIllustrationExtractor(pdf: PDFDocumentProxy) {
  ...
  async function extract(pageNumber: number): Promise<ReaderIllustration[]> {
    try {
      const [{ OPS, ImageKind }, page] = await Promise.all([import("pdfjs-dist"), pdf.getPage(pageNumber)]);
      const operatorList = await page.getOperatorList();
      const candidates = findIllustrationCandidates(page, operatorList, OPS);
      ...
      for (const candidate of candidates) {
        ...
        if (hasTextOverImage(candidate.bounds, text.items)) continue;
  ...
  return {
    get(pageNumber: number) {
      ...
      const pending = extract(pageNumber);
